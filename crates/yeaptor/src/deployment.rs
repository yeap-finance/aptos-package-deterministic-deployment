use crate::config::{Deployment, YeaptorConfig, load_config};
...
impl YeaptorEnv {
    ...
    pub fn build_all(
        ...
        let deployments = self
            ...
            .deployments
            .iter()
            .map(|deployment| self.build_deployment(deployment, included_args, move_options))
    ...
    async fn execute(self) -> CliTypedResult<String> {
        ...
        for dep in built_deployments {
            for (pkg_name, metadata_serialized, modules) in &dep.packages {
                ...
                let out_path = self
                    .out_dir
                    .join(format!("{}-{}-publish.json", written, pkg_name));
